 ...
 * @param {string} loginUrl - URL to redirect to if not authenticated (default: /login-working.html)
 * @param {number} retries - Number of retry attempts (default: 3)
 * @returns {Promise<object>} Current user if authenticated
 */
async function setupAuthGuard(loginUrl = '/login-working.html', retries = 5) {
    try {
...
/**
 * Logout current user
 * @param {string} redirectUrl - URL to redirect after logout (default: /login-working.html)
 * @returns {Promise<boolean>} Success status
 */
async function logout(redirectUrl = '/login-working.html') {
    try {
        if (!window.supabaseClient) {
            throw new Error('Supabase client not initialized');
        }
...
function setupAuthListener(callback) {
    ...
    const { data: { subscription } } = window.supabaseClient.auth.onAuthStateChange((event, session) => {
        ...
        if (event === 'SIGNED_OUT') {
            window.location.replace('/login-working.html');
        }
