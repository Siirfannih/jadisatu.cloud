...
export default function DashboardSidebar({ currentView, onSelectView, focusCount = 3, crmCount = 12, agentsOnline = 2 }: Props) {
  ...
  const navClass = (view: ViewId) =>
    cn(
      ...
      currentView === view ? 'bg-muted text-foreground' : 'text-muted-foreground hover:text-foreground hover:bg-muted'
    )

  return (
    <aside className={cn('bg-card border-r border-border flex flex-col shrink-0 z-20 transition-all duration-300', collapsed ? 'w-16' : 'w-64')}>
      {/* Profile Header */}
      <div className="p-6 border-b border-border">
        <div className="flex items-center gap-3 mb-4">
          ...
          {!collapsed && (
            <div>
              <div className="font-semibold text-foreground">Irfan</div>
          ...
          <>
            <div className="space-y-2">
              ...
              <div className="flex gap-3 text-[10px]">
                <span className="flex items-center gap-1 text-blue-600 dark:text-blue-400"><div className="w-1.5 h-1.5 rounded-full bg-work" />Work</span>
                <span className="flex items-center gap-1 text-amber-600 dark:text-amber-400"><div className="w-1.5 h-1.5 rounded-full bg-learn" />Learn</span>
                <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400"><div className="w-1.5 h-1.5 rounded-full bg-business" />Business</span>
              </div>
            </div>
          </>
        )}
      ...
      <div className="flex-1 overflow-y-auto custom-scroll py-4 px-3 space-y-1">
        {!collapsed && <div className="px-3 mb-2 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Dashboard</div>}
        ...
        {!collapsed && <div className="mt-6 px-3 mb-2 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Domains</div>}
        <button onClick={() => onSelectView('domains')} className={navClass('domains')}>
          <div className="w-2 h-2 rounded-full bg-work shrink-0" />
          {!collapsed && <><span className="flex-1 text-left">Work & Career</span><span className="text-[10px] text-muted-foreground/70">Ojek</span></>}
        </button>
        <button onClick={() => onSelectView('domains')} className={navClass('domains')}>
          <div className="w-2 h-2 rounded-full bg-learn shrink-0" />
          {!collapsed && <><span className="flex-1 text-left">Learning</span><span className="text-[10px] text-muted-foreground/70">Kuliah</span></>}
        </button>
        <button onClick={() => onSelectView('domains')} className={navClass('domains')}>
          <div className="w-2 h-2 rounded-full bg-business shrink-0" />
          {!collapsed && <><span className="flex-1 text-left">Business</span><span className="text-[10px] text-muted-foreground/70">JadiSatu</span></>}
        </button>

        {!collapsed && <div className="mt-6 px-3 mb-2 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Workspace</div>}
        ...
        <button onClick={() => onSelectView('notes')} className={navClass('notes')}>
          ...
          {!collapsed && <><span className="flex-1 text-left">Notes & Ideas</span><span className="text-[10px] text-muted-foreground/70">Notion-style</span></>}
        </button>
        <button onClick={() => onSelectView('crm')} className={navClass('crm')}>
          <Users className="w-4 h-4 shrink-0" />
          {!collapsed && <><span className="flex-1 text-left">CRM</span><span className="bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-[10px] px-2 py-0.5 rounded-full">{crmCount}</span></>}
        </button>

        {!collapsed && <div className="mt-6 px-3 mb-2 text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">System</div>}
      ...
      <div className="p-4 border-t border-border space-y-2">
        <button className={cn('w-full flex items-center gap-2 px-3 py-2 rounded-lg bg-muted hover:bg-muted/80 text-xs text-muted-foreground hover:text-foreground transition-all', collapsed && 'justify-center')}>
